...
public class FileManager {
	...
	public String doFileUpload(MultipartFile partFile, String pathname) throws Exception {
		...
		// 확장자
		// 파일명을 변경하기 위해 마지막 "."을 찾아서 확장자를 찾는다.
		String fileExt = originalFilename.substring(originalFilename.lastIndexOf("."));
		if(fileExt == null || fileExt.equals(""))
			return null;
		
		// 서버에 저장할 새로운 파일명을 만든다. (년월일시분초NS)
	...
	public boolean doFileDownload(String saveFilename, String originalFilename, String pathname, HttpServletResponse response) {
	    ...
	    try {
	        ...
	        if (file.exists()){
	            ...
	            // Server > Client한테 전송할때 Header에 전송
				response.setHeader(
						"Content-disposition",
	...
	/**
	 * 파일을 삭제하는 메소드 (파일은 1개만 삭제 가능, 폴더는 비워있지 않으면 삭제 불가)
